...
js/
  ...
  sync-codec.js     -- 跨端数据联通纯函数核心（规范 schema/词书 key 映射/卡片与日志合并/分块协议，与小程序同语义）
  sync-ui.js        -- 同步面板 UI + Web 端收集/应用适配（文件导出/导入；二期配对码云通道）
data/
...
## 最近更新（2026-08-23）— 数据联通一期（文件通道，对齐小程序）

**跨端同步 codec**：新增 `js/sync-codec.js`（App.SyncCodec 纯函数：规范 schema v1、词书 key 映射 web→canonical=小程序 key、cardMerge/logMerge/settingsMerge、chunkPayload/assembleChunks 64KB+FNV-1a 分块协议、旧 data-io 备份格式自动转换）与 `js/sync-ui.js`（头部 🔄 同步弹窗 + 收集/应用适配 + Blob 导出/文件导入）。**合并语义**：卡片按 (词书 source, lower(word)) 锚定取较优（stage/nextReview/history 并集截断 100/wrongDates 并集/isHard OR）；日志 M = max(本地, 快照) + max(0, 远端 − 远端上次全量)（状态按发送端 deviceKind 分槽存 `flashcard-sync-log-state`，防重复累计/重复导入不动点/多轮收敛）；设置以导出端为准（en-US/en-GB↔us/uk 归一）。**共享测试向量**单一来源 mini `tests/data/sync-vectors.js`，web 测试经 `../../../English-mini-app/tests/data/sync-vectors.js` 跨仓引用（子模块缺失优雅跳过）

**存储适配**：idb-storage 精简格式补 `ih`（isHard 生词本收藏，web 暂无 UI）+ deck `source` 持久化（同步锚点，此前 IDB 恢复会丢 source）

**UI**：index.html 头部 🔄 按钮 + syncModal（复用 import-modal 样式）；components.css 追加 `.sync-actions`；未导入词书整包跳过并计入摘要；build.js JS_ORDER 插入 sync-codec/sync-ui
...
**逻辑零改动**：预取/source 判定/去重/分区信息结构全部保持；测试 294 全绿零改动（类名保留策略）；SW 缓存 v13→v14
...
- `flashcard-sync-log-state` — 数据同步日志合并状态（按发送端分槽 {web|mini: {snap, remotePrev}}，防重复累计基线）

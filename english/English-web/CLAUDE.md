...
- **⌨️ 拼写模式（纯练习）**：学习中内嵌；**盲拼**（正面下划线对应单词字母数 `_ _ _ _ _ _ _`（带间隔，按空白分组，连字符/撇号原样保留）+ 辅助提示，词形/音标/难度/计数点全隐藏，点卡片不翻转防泄漏背面释义）；对/错均停留当前词（✅/❌ 反馈、可重拼/重试），**不推进队列、不改遗忘曲线、不写学习日志**；拼写期间隐藏会了/不会按钮，退出后恢复作答；遮罩生成纯函数 `buildSpellBlindMask`

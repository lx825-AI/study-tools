...
- **file:// 协议 Service Worker 同步抛异常**：Chrome 中 `navigator.serviceWorker.register()` 在 file:// 下同步抛出 TypeError，`.catch()` 无法捕获，导致后续 JS 初始化代码不执行。修复方法：外层加 `try-catch`（index.html 和 scripts/build.js 同步处理）
